...
{
    /// <summary>
    ///     The <see cref="Command{T}"/> implementation to toggle the camera mode of a <see cref="HelixViewport3D"/> between orthographic and perspective
    /// </summary>
    public class ToggleViewportCameraCommand : Command<HelixViewport3D>
    {
        ...
        public override void Execute(HelixViewport3D parameter)
        {
            ...
            var newCamera = parameter.Camera.GetType().IsAssignableFrom(typeof(PerspectiveCamera)) 
                ? (ProjectionCamera) new OrthographicCamera() : new PerspectiveCamera();

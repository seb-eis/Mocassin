...
namespace Mocassin.Mathematics.Solver
{
    ...
    public class PointMechanicsSolver
    {
        ...
        /// <returns></returns>
        public List<T1> ShiftOriginToMassCenter<T1>(IEnumerable<T1> original, IComparer<double> comparer)
            where T1 : struct, ICartesianMassPoint3D<T1>
        {
            if (!(original is ICollection<T1> orgCollection))
                orgCollection = original.ToList();

            var massCenter = GetMassCenter(orgCollection, comparer ?? Comparer<double>.Default);
            return orgCollection
                .Select(value => value.CreateNew(value.Vector - massCenter, value.GetMass()))
                .ToList();
        }
        ...
        /// <param name="comparer"></param>
        /// <returns></returns>
        public Cartesian3D GetMassCenter<T1>(IEnumerable<T1> massPoints, IComparer<double> comparer)
            where T1 : struct, ICartesianMassPoint3D<T1>
        {
            if (comparer == null) 
        ...
        /// <param name="massPoints"></param>
        /// <returns></returns>
        public Matrix2D GetInertiaTensor<T1>(IEnumerable<T1> massPoints) 
            where T1 : struct, ICartesianMassPoint3D<T1>
        {
        ...
        /// <param name="massPoints"></param>
        /// <returns></returns>
        public double GetSumOfMassTimesDistance<T1>(IEnumerable<T1> massPoints) 
            where T1 : struct, ICartesianMassPoint3D<T1>
        {
        ...
        /// <param name="comparer"></param>
        /// <returns></returns>
        public MassPointGeometryInfo CreateGeometryInfo<T1>(IEnumerable<T1> massPoints, IComparer<double> comparer)
            where T1 : struct, ICartesianMassPoint3D<T1>
        {
        ...
        protected void AddInertiaEntry<T1>(Matrix2D tensor, T1 point) 
            where T1 : struct, ICartesianMassPoint3D<T1>
        {

...
namespace ICon.Model.Basic
{
    ...
    public abstract class DataConflictHandlerProvider<T1> : IDataConflictHandlerProvider<T1> where T1 : ModelData
    {
        /// <summary>
        ...
        /// <summary>
        /// Interface access to the conflict resolver for new model objects
        /// </summary>
        ...
        /// <summary>
        /// Interface access to the conflict resolver for changed model objects
        /// </summary>
        ...
        /// <summary>
        /// Interface access to the conflict resolver for removed model objects
        /// </summary>
        ...
        /// <summary>
        /// Interface access to the conflict resolver for reindexed model object lists
        /// </summary>
        ...
        /// <summary>
        /// Interface access to the conflict resolver for change model parameter objects
        /// </summary>
        ...
        /// <summary>
        /// Searches the class for all conflict resolver creation methods and assigns them to their affiliated property. Assigns empty resolvers if no creation method is specified
        /// </summary>
        protected virtual void InitializeConflictResolvers()
        {
            var flags = BindingFlags.Public | BindingFlags.Instance;
            foreach (var propertyInfo in GetType().GetProperties(flags).Where(property => property.GetCustomAttribute(typeof(ConflictHandlerAttribute)) != null))
            {
        ...
        /// <summary>
        /// Auto assigns a matching resolver (Or assigns a dummy if no resolver source is found in the source methods) to the property described by the passed property info
        /// </summary>
        ...
        protected virtual void AutoAssignResolver(PropertyInfo propertyInfo)
        {
            ...
            {
                throw new ArgumentException("The passed resolver property is not null, possibly multiple resolver source definitions", nameof(propertyInfo));
            }
        ...
        /// <summary>
        /// Takes a property info that belongs to a data conflict resolver interface and creates a dummy handler (No resolving required) matching the generic arguments
        /// </summary>
        /// <param name="propertyInfo"></param>
        /// <returns></returns>
        protected virtual object MakeEmptyResolver(PropertyInfo propertyInfo)
        {
            return Activator.CreateInstance(typeof(DataConflictHandlerDummy<,>).MakeGenericType(propertyInfo.PropertyType.GetGenericArguments()));
        }

        /// <summary>
        /// Finds the method that is marked as a resolver source for the passed resolver property info and returns a delegate for it. Returns null if none is found
        /// </summary>
        /// <param name="propertyInfo"></param>
        /// <returns></returns>
        protected virtual Func<object> FindResolverSourceMethod(PropertyInfo propertyInfo)
        {
            var operationType = ((ConflictHandlerAttribute)propertyInfo.GetCustomAttribute(typeof(ConflictHandlerAttribute))).DataOperationType;
            ...
            {
                if (@delegate.Method.ReturnParameter.ParameterType != typeof(object))
                {
                    throw new InvalidOperationException("Marked provider delegate does not return an object and cannot be cast to Func<object>");
                }
                if (@delegate.Method.GetParameters().Length != 0)
                {
                    throw new InvalidOperationException("Marked provider delegate does not have an empty parameter list and cannot be cast to Func<object>");
                }

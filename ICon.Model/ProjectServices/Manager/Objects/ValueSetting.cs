...
namespace ICon.Model.ProjectServices
{
    ...
    public class ValueSetting<T> where T : IComparable<T>
    {
        ...
        /// <summary>
        /// Creates new value restriction with display name and all four restricable values
        /// </summary>
        ...
        public ValueSetting(string displayName, T minValue, T lowerWarningLimit, T upperWarningLimit, T maxValue)
        {
            ...
            SetValues(new T[] { minValue, maxValue, upperWarningLimit, lowerWarningLimit });
        }
        ...
        /// <param name="minValue"></param>
        /// <param name="maxValue"></param>
        public ValueSetting(string displayName, T minValue, T maxValue) : this(displayName, minValue, minValue, maxValue, maxValue)
        {
        ...
        /// <summary>
        /// Parses the value and checks for limitation violations and supplies affiliated warning messages. Returned integer indicates level of violation
        /// </summary>
        ...
        {
            int parseResult = ParseValue(value);
            var warningList = new List<WarningMessage>(2);

            if (Math.Abs(parseResult) == 1)
            {
                var detail0 = $"The [{DisplayName}] value ({value}) is outside of the warning boundaries [{LowerWarningLimit},{UpperWarningLimit}]";
        ...
        /// <summary>
        /// Parses the value and returns 0 if the valu is valid. Returns +/- 1 if the warning limit is reached, returns +/- 2 if the limitation is violated
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        public int ParseValue(T value)
        {
            int lowerParse = ParseLower(value);
            if (lowerParse == 0)
            {
                return ParseUpper(value);
            }
            return lowerParse;
        }

        /// <summary>
        /// Parses the value for violation of upper warning limit or restriction value. Returns 0, 1 or 2 depedning on the violation
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Parses the value for violation of lower warning limit or restriction value. Returns 0, -1 or -2 depedning on the violation
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>

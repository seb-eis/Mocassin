...
namespace Mocassin.Model.Transitions
{
    ...
    /// </summary>
    public class TransitionRuleGenerator_Deprecated<TRule> : ITransitionRuleGenerator<TRule> where TRule : TransitionRule, new()
    {
        /// <summary>
        ...
        public TransitionRuleGenerator_Deprecated(IEnumerable<IParticle> particlePool)
        {
        ...
        public IEnumerable<IEnumerable<TRule>> MakeUniqueRules(IEnumerable<IAbstractTransition> abstractTransitions, bool onlySupported)
        {
            ...
            var unfiltered = MakeUniqueRules(abstractCollection, statePairGroups);
            var result = onlySupported
                ? unfiltered.Select(FilterByCommonBehaviorAndInversionRules)
                : unfiltered;
            return result;
        }
        ...
        protected void FilterInvertedAndReversedDuplicateRules(IList<TRule> rules)
        {
            ...
            for (var i = 0; i < rules.Count - 1; i++)
            {
                ...
                for (var j = i + 1; j < rules.Count;)
                {
                    ...
                    var isRemovable = analyzer.RulePairIsDependentPair(parentRule, checkRule);

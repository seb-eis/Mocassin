...
{
    /// <summary>
    ///     Geometry hash object that is used to do an approximated symmetry comparisons between groups of mass points based on
    ///     hash values
    /// </summary>
    public readonly struct SymmetryIndicator
    {
        ...
        public SymmetryIndicator(double first, double second)
            : this()
        {
            First = first;
            Second = second;
        ...
        /// </summary>
        /// <returns></returns>
        public override string ToString() => $"Symmetry Indicator ({First}, {Second})";
        ...
        public static Comparer<SymmetryIndicator> MakeComparer(IComparer<double> comparer)
        {
            ...
            {
                var first = comparer.Compare(lhs.First, rhs.First);
                if (first == 0) return comparer.Compare(lhs.Second, rhs.Second);
                return first;
            }

...
namespace Mocassin.Symmetry.CrystalSystems
{
    ...
    public abstract class CrystalSystem
    {
        ...
        /// <summary>
        ///     Checks if three radians angles are basically valid crystal system angles without specified crystal system type
        /// </summary>
        ...
        {
            return ArePossibleCellAngles(alpha, beta, gamma);
        ...
        /// <summary>
        ///     Checks if three radians angles can form a valid crystal geometry
        /// </summary>
        /// <param name="alpha"></param>
        /// <param name="beta"></param>
        /// <param name="gamma"></param>
        /// <returns></returns>
        public bool ArePossibleCellAngles(double alpha, double beta, double gamma)
        {
            var val0 = alpha + beta + gamma;
            var con0 = val0 > 0 && val0 < Math.PI * 2.0;
            var val1 = alpha + beta - gamma;
            var con1 = val1 > 0 && val1 < Math.PI * 2.0;
            var val2 = alpha - beta + gamma;
            var con2 = val2 > 0 && val2 < Math.PI * 2.0;
            var val3 = beta + gamma - alpha;
            var con3 = val3 > 0 && val3 < Math.PI * 2.0;
        ...
        public bool ValidateAngleConstraintCondition(double alpha, double beta, double gamma) =>
            AlphaConstraint.IsValid(alpha) && BetaConstraint.IsValid(beta) && GammaConstraint.IsValid(gamma);
        ...
        public bool ValidateParameterConditions(double paramA, double paramB, double paramC) =>
            ValidateParameterConstraintCondition(paramA, paramB, paramC)
        ...
        public bool ValidateParameterConstraintCondition(double paramA, double paramB, double paramC) =>
            ParameterConstraint.IsValid(paramA) && ParameterConstraint.IsValid(paramB) && ParameterConstraint.IsValid(paramC);
        ...
        /// </summary>
        /// <returns></returns>
        public (Coordinates3D A, Coordinates3D B, Coordinates3D C) CalculateBaseVectors() => (GetLatticeVectorA(), GetLatticeVectorB(), GetLatticeVectorC());

        /// <summary>
        ///     Calculates the lattice vector in A direction from the system parameters
        /// </summary>
        /// <returns></returns>
        public Coordinates3D GetLatticeVectorA() => new Coordinates3D(ParamA.Value, 0.0, 0.0);
        ...
        public bool BasicVectorValidation(Coordinates3D vectorA, Coordinates3D vectorB, Coordinates3D vectorC) =>
            vectorA.IsLinearIndependentFrom(vectorB, vectorC, BasicConstraint.Comparer);
        ...
        /// <returns></returns>
        protected bool ValidateVectorLength(in Coordinates3D vector) => !BasicConstraint.Comparer.Equals(vector.GetLength(), 0.0);

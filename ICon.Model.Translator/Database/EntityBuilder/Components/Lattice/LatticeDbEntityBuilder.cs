...
namespace Mocassin.Model.Translator.EntityBuilder
{
	public class LatticeDbEntityBuilder : DbEntityBuilder, ILatticeDbEntityBuilder
	{
		...
		{
			var rawLattice = LatticeCreationProvider.BuildLattice(latticeConfiguration.GetIntVector3D(), latticeConfiguration.DopingConcentrations, Rng);
		...
		public InteropObject<CLatticeInfo> GetLatticeInfoEntity(LatticeEntity latticeEntity, ISimulationModel simulationModel)
		{
			...
			CLatticeInfo latticeInfo = new CLatticeInfo() {SizeVector = new CVector4(latticeDimensions[0], latticeDimensions[1], latticeDimensions[2], latticeDimensions[3])};
		...
		private void CountNumberOfMobiles(LatticeEntity latticeEntity, ref CLatticeInfo latticeInfo, ISimulationModel simulationModel)
		{
			...
			for (int atomIndex = 0; atomIndex < latticeEntity.Length;)
			{
				for (int positionIndex = 0; positionIndex < latticeInfo.SizeVector.D; positionIndex++)
				{

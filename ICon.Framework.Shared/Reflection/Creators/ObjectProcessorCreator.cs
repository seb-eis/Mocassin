...
namespace ICon.Framework.Reflection
{
    ...
    public class ObjectProcessorCreator
    {
        ...
        /// <summary>
        /// Searches the provided class isntance for methods that match the prediacte and binding flags and returns a sequence of object processors for all found methods
        /// </summary>
        ...
        /// <returns></returns>
        public IEnumerable<IObjectProcessor<TResult>> CreateProcessors<TResult>(object instance, Predicate<MethodInfo> predicate, BindingFlags bindingFlags)
        {
            return new DelegateCreator().CreateDelegates(instance, predicate, bindingFlags).Select(@delegate => (IObjectProcessor<TResult>)CreateProcessor(@delegate));
        }

        /// <summary>
        /// Searches the provided class isntance for methods that match the prediacte and binding flags and returns a sequence of sync object processors fo all found methods
        /// </summary>
        /// <typeparam name="TResult"></typeparam>
        /// <param name="instance"></param>
        /// <param name="predicate"></param>
        /// <param name="bindingFlags"></param>
        /// <returns></returns>
        public IEnumerable<IAsyncObjectProcessor<TResult>> CreateAsyncProcessors<TResult>(object instance, Predicate<MethodInfo> predicate, BindingFlags bindingFlags)
        {
            return new DelegateCreator().CreateDelegates(instance, predicate, bindingFlags).Select(@delegate => (IAsyncObjectProcessor<TResult>)CreateAsyncProcessor(@delegate));
        }
        ...
        {
            var paramTypes = @delegate.Method.GetParameters().Select(param => param.ParameterType).ToList();
        ...
        protected object CreateAsyncProcessor(Delegate @delegate)
        {
            var paramTypes = @delegate.Method.GetParameters().Select(param => param.ParameterType).ToList();
            paramTypes.Add(@delegate.Method.ReturnType);
            return Activator.CreateInstance(GetRawAsyncProcessorType(paramTypes.Count - 1).MakeGenericType(paramTypes.ToArray()), @delegate);
        }

        /// <summary>
        /// Get the raw type for an object processor with the specififed number of handled objects
        /// </summary>
        ...
        /// <summary>
        /// Get the raw type for an async object processor with the specififed number of handled objects
        /// </summary>
        /// <param name="objectCount"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Get an array that contains the supported raw processor types (index equals number of handled objects, index 0 is always null)
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Get an array that contains the supported raw async processor types (index equals number of handled objects, index 0 is always null)
        /// </summary>
        /// <returns></returns>

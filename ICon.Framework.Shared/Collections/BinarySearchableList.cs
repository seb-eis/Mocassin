...
namespace Mocassin.Framework.Collections
{
    ...
    public abstract class BinarySearchableList<T1> : IList<T1>
    {
        ...
        /// <inheritdoc />
        public bool Contains(T1 item) => List.BinarySearch(item, Comparer) >= 0;
        ...
        /// </summary>
        public (int Start, int End) GetCppEnclosingBounds(T1 item) => (GetCppLowerBound(item), GetCppUpperBound(item));

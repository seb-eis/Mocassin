...
{
    /// <summary>
    /// Defines extension methods for the IObservable<T> interface of the reactive extensions
    /// </summary>
    public static class IConObservableExtensions
    {
        /// <summary>
        ...
        {
            return Observable.Create<T1>(obs => source.Subscribe(x => { spyNext(x); obs.OnNext(x); }, ex => obs.OnError(ex), () => obs.OnCompleted()));
        }
        ...
        {
            return Observable.Create<T1>(obs => source.Subscribe(x => obs.OnNext(forger(x)), ex => obs.OnError(ex), () => obs.OnCompleted()));
        }
        ...
        /// <returns></returns>
        public static IObservable<T1> AllowIf<T1>(this IObservable<T1> source, Func<T1, Boolean> passCondition)
        {
            return Observable.Create<T1>(obs => source.Subscribe(x => { if (passCondition(x)) { obs.OnNext(x); } }, ex => obs.OnError(ex), () => obs.OnCompleted()));
        }
        ...
        /// <returns></returns>
        public static IObservable<T1> BlockIf<T1>(this IObservable<T1> source, Func<T1, Boolean> blockCondition)
        {
            return Observable.Create<T1>(obs => source.Subscribe(x => { if (!blockCondition(x)) { obs.OnNext(x); } }, ex => obs.OnError(ex), () => obs.OnCompleted()));
        }
